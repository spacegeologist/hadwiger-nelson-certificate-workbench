...
## Exact Geometry Certificates

`scripts/verify_embedding.py` checks `|dist - 1| < tol` numerically. For a
proof-grade certificate, `scripts/verify_embedding_exact.py` verifies each edge
has squared length *identically* equal to 1 in the exact algebraic number field
`Q(sqrt 3, sqrt 5, sqrt 11)` generated by the CNP-SAT coordinates:
...
Expected: `result: all edges are exactly unit distance`. All four CNP-SAT graphs
(510, 517, 529, 553) pass exactly.

## Deletion Minimization

`scripts/minimize_graph.py` greedily deletes edges while an incremental SAT
solver proves the graph stays non-4-colorable, then re-verifies the result on a
fresh solver and checks the surviving edges are still unit distance:

```bash
python3 scripts/minimize_graph.py \
  --edge data/hadwiger-nelson/external/marijnheule-CNP-SAT/edge/510.edge \
  --vtx  data/hadwiger-nelson/external/marijnheule-CNP-SAT/vtx/510.vtx \
  --order lexicographic --conflict-budget 8000 \
  --out-edge   data/hadwiger-nelson/generated/minimized/510-min.edge \
  --out-vtx    data/hadwiger-nelson/generated/minimized/510-min.vtx \
  --report     data/hadwiger-nelson/generated/minimized/510-min.report.json
```

Note: the 510 graph is *vertex-critical*, so deletion cannot reduce the vertex
